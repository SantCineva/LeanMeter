...
class SSD1306_GFX:
    ...
    ### Method to display .pbm files on the OLED ###
    # Figures by itself the image resolution from the PBM file contents
    #
    # filePath  ->  path to file on the MCU
    
    def display_pbm(self, filePath):
        with open(filePath, 'rb') as f:
            # Read the PBM header
            header = f.readline()
            if header.strip() != b'P4':
                raise ValueError("Unsupported PBM format. Only binary PBM (P4) is supported.")
        ...
        # Create a frame buffer to hold the image
        fb = framebuf.FrameBuffer(bitmap, width, height, framebuf.MONO_HLSB)
        
        # Clear the OLED display
        self.fill(0)
        # Blit the frame buffer to the OLED display
        self.blit(fb, 0, 0)
        # Update the display
        self.show()
        
        
    ### Play animation using .pbm files ###
    # Needs the method display_pbm in order to be able to load the file format
    #
    # inputFolder  ->  folder on the mcu containing the PMB files
    # frameTime  ->  the langth of time an image will be kept on before the next one
    
    def play_animation(self, inputFolder, frameTime):
        # List all PBM files in the folder
        files = [f for f in os.listdir(inputFolder) if f.endswith('.pbm')]
        
        for file in files:
            filePath = inputFolder + '/' + file
            self.display_pbm(filePath)
...
if __name__ == "__main__":
    ...
    ssd1306_display = SSD1306_I2C_SETUP(1, 0, 128, 64)
